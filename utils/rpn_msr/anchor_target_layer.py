...
def anchor_target_layer(image,rpn_cls_score, gt_boxes, im_info, _feat_stride=[16, ], anchor_scales=[16, ]):
    ...
    gt_boxes: (G, 8) vstack of [x1, y1, x2, y2, ...]
    ...
    #
    image_ = image.copy()
    for item in anchors[fg_inds]:
        cv2.rectangle(image_, (int(item[0]), int(item[1])), (int(item[2]),int(item[3])),(0,0,255))

    cv2.imwrite('result_before_throw.jpg', image_)
    #notice fzh 改为正样本　不限制
    ...
    for item in fg_auchors:
        image = cv2.rectangle(image, (int(item[0]), int(item[1])), (int(item[2]),int(item[3])),(0,0,255))
    ...
    for item in bg_auchors:
        image = cv2.rectangle(image, (int(item[0]),int(item[1])),(int(item[2]),int(item[3])),(0,255,255))
    cv2.imwrite('result_bg_fg.jpg', image)
